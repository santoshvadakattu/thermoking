...
public class SearchServiceImpl implements SearchService {
	...
	public void activate() {
		LOG.info("/n----------ACTIVATE METHOD-----");
	}

	public Map<String, String> crateTextSearchQuery(String SearchText, int startResult, int resultPerPage) {
		Map<String, String> queryMap = new HashMap<>();
		queryMap.put("path", "/content/thermoking");
		queryMap.put("type", "cq:page");
		queryMap.put("fulltext", SearchText);
		queryMap.put("p.offset", Long.toString(startResult));
		queryMap.put(SearchText, Long.toString(resultPerPage));
		return queryMap;
	}

	@Override
	public JSONObject searchResult(String searchText, int startResult, int resultPerPage) {
		LOG.info("/n-----SEARCH RESULT-----");
		JSONObject searchResult = new JSONObject();
		try {
			ResourceResolver resourceResolver = ResolverUtil.newResolver(resourceResolverFactory);
			final Session session = resourceResolver.adaptTo(Session.class);
			Query query = queryBuilder.createQuery(
					PredicateGroup.create(crateTextSearchQuery(searchText, startResult, resultPerPage)), session);
...
//        double totalPages = Math.ceil((double) totalResults / (double) resultPerPage);
			...
			searchResult.put("perpageResults", perPageResult);
